...
CREATE TABLE IF NOT EXISTS records (
  ...
  CONSTRAINT domain_fkey FOREIGN KEY (domain_id) REFERENCES domains(id) ON DELETE CASCADE
);
...
-- Create database for PowerDNS-Admin if it doesn't exist
CREATE DATABASE IF NOT EXISTS powerdns_admin;

-- Grant permissions
GRANT ALL PRIVILEGES ON DATABASE powerdns TO powerdns;
GRANT ALL PRIVILEGES ON DATABASE powerdns_admin TO powerdns;
